...
public class HttpSessionIQMService implements HttpSessionIQMServiceInterface {
    ...
    private Stub resumeStub(boolean resumeTransaction) {
        try {
            ...
            } else if(stub.transaction!=null || stub.transaction.getStatus()!=Status.STATUS_NO_TRANSACTION) {
                throw new InvalidHttpSessionServiceException("Transaction must be resumed");
            }
            return stub;
...
//    }
    ...
    public void init(String userName, String pwd) {
        ...
        if (service != null) {
            throw new InvalidHttpSessionServiceException("service is not null");
        }
        ...
        try {
            ...
            setServiceStub(session, service);
        } catch (InvalidRMIServiceException ex) {
            throw new InvalidHttpSessionServiceException(ex);
        }
        ...
        try {
            ...
            if(transaction== null )
                throw new InvalidHttpSessionServiceException("fail to get a valid Transaction");
            else if( transaction.getStatus()!=Status.STATUS_NO_TRANSACTION)
                throw new InvalidHttpSessionServiceException("Transaction status ("+transaction.getStatus()+")is not Status.STATUS_NO_TRANSACTION");
            else
                setTransactionStub(session, transaction);
        } catch (NamingException | SystemException ex) { 
            throw new InvalidHttpSessionServiceException(ex);
        } 
    }
...
//    }
    ...
    @WebMethod
    @Override
    public void beginTransaction(int timeOutSeconds) {
        //only resume the stub, without resume
        Stub stub = this.resumeStub(false);
        
        try {
            stub.transactionManager.setTransactionTimeout(timeOutSeconds);
...
//
    ...
    @WebMethod
    @Override
    public String getQuoteVOByPermId(Long permId, Date effectiveFrom, Date effectiveTo) {
        Stub stub = this.resumeStub(true);
        String res = null;
        try {
            QuoteVO quoteVO = stub.rmiService.getQuoteVOByPermId(permId, effectiveFrom, effectiveTo);
